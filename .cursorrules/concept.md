...
```
Mobile:      React Native with Expo (SDK 51+), TypeScript
...
EXPO_PUBLIC_HELIUS_RPC=https://devnet.helius-rpc.com/?api-key=YOUR_KEY
EXPO_PUBLIC_API_URL=https://api.rizq.app
EXPO_PUBLIC_SAVINGS_PROGRAM_ID=YOUR_ANCHOR_PROGRAM_ID
EXPO_PUBLIC_COMMITTEE_PROGRAM_ID=YOUR_ANCHOR_PROGRAM_ID
EXPO_PUBLIC_PAYOUT_PROGRAM_ID=YOUR_ANCHOR_PROGRAM_ID
...
- On submit: call Supabase Auth signInWithOtp({ phone })
...
- Auto-reads SMS on Android using expo-sms-reader
- "Resend code" link (disabled for 60s countdown, shows timer)
- "Edit number" link goes back to phone screen
- On complete: call Supabase Auth verifyOtp({ phone, token, type: 'sms' })
...
- On submit: upload photos to Supabase Storage, insert into users table with kyc_status='pending'
...
- Check kyc_status on Supabase real-time subscription — if changes to 'verified', show success toast and enable features
```
...
- On tap:
  ...
  3. Insert committee record in Supabase with pda_address
...
- Fetch committee data from Supabase using invite_code
- Show preview card:
...
Data to fetch on mount:
  - Committee record from Supabase
...
5. EMERGENCY CONTROLS (bottom, red section):
   ...
   "Export full history" (gray) — generates PDF via backend API
```
...
- Share milestone card: "Export achievement image" → generates shareable PNG with Rizq branding
...
- Persist last 20 messages in Supabase
...
- Your Solana wallet address in a large QR code (expo-qrcode or react-native-qrcode-svg)
...
- Feature comparison table:
  ...
  - Export history (not available vs PDF + CSV)
...
const HELIUS_RPC = process.env.EXPO_PUBLIC_HELIUS_RPC!;
...
const USDC_MINT = new PublicKey(process.env.EXPO_PUBLIC_USDC_MINT!);
...
GET    /api/committees/:id/export   — Generate PDF export
...
cron.schedule('0 5 * * 0', async () => {
  ...
  for (const member of activeMembers.data) {
    ...
    await sendPushNotification(member.users.expo_push_token, 'Rizq weekly update', message);
  }
});
```
...
export const formatUSDC = (microUnits: number): string => {
  return `$${(microUnits / 1_000_000).toFixed(2)} USDC`;
};

export const formatPKR = (usdcAmount: number, pkrRate: number): string => {
...
export const formatDaysLeft = (deadline: Date): string => {
...
export const formatCycleOrdinal = (n: number): string => {
  const s = ['th','st','nd','rd'];
  const v = n % 100;
...
Week 3: AI + Real Data
  [ ] Supabase queries replacing all static data
